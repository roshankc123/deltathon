...
def predict(image, crop):
  ...
  img = img.to(device)
  if crop=='rice':
    model, state_dict = load_model('models/disease/rice.pth', 3)
    model.load_state_dict(state_dict)
    
    # classes = 
    # backend/static/schedule.py
  elif crop == 'maize':
    model, state_dict = load_model('models/disease/maize.pth', 4)
    model.load_state_dict(state_dict)  
    classes = Maize_classes
    # from static.disease import Maize as dictDisease
    from statics.disease import Maize as dictDisease
    # print(dictDisease)
  elif crop=='apple':
    model, state_dict = load_model('models/disease/apple.pth', 4)
    model.load_state_dict(state_dict)
    classes = apple_classes
    from statics.disease import Apple as dictDisease
    # print(dictDisease)
  elif crop=='potato':
    model, state_dict = load_model('models/disease/potato.pth', 3)
    model.load_state_dict(state_dict)
    classes = Potato_classes
    from statics.disease import Potato as dictDisease
    # print(dictDisease)
  output = model(img.unsqueeze_(0))
  pred, conf = output.max(-1)
  fclass = list(classes.keys())
  # print(fclass[conf.item()])
  return { 'description' : dictDisease[fclass[conf.item()]], 'confidence' : pred.item() * 100, 'crop' : crop }

# print(predict('image/temp.jpeg', 'potato'))